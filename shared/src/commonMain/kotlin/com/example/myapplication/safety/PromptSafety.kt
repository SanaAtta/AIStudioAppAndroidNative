...
object PromptSafety {
    ...
    init {
        loadTerms(PromptSafetyBlockedWords.ALL)
    }
    ...
    fun requireSafe(vararg texts: String) {
        if (texts.any { containsUnsafeContent(it) }) {
            throw IllegalArgumentException(UNSAFE_MESSAGE)
        }
    }

    fun loadTerms(terms: Collection<String>) {
        val cleaned = terms
     ...
     * a person lying on a bed with legs spread, inviting a partner to come closer
     * (including romanized Urdu/Hindi wording).
     */
    ...
    private fun matchesProhibitedScene(text: String): Boolean {
        ...
        val hasInvite = Regex(
            """\b(husband|wife|partner|come here|come to me|aa jao|aajao|ajawo|emotion nikalo|""" +
