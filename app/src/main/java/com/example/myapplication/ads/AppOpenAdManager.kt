 ...
 * Resume app-open: single load after the welcome overlay via [loadAndShowResumeAppOpen].
 */
object AppOpenAdManager {
    ...
    /**
     * After the welcome overlay: reuse a warm cache when possible; otherwise one load + show.
     */
    ...
    private fun showResumeInternal(
        ...
        onAdReadyToShow?.run()
        ad.show(activity)
        AdsLoadLog.shown("resume app open ad", AdUnitIds.Rc.APP_OPEN_RESUME, AdUnitIds.appOpenResume())
    }
    ...
    private fun showSplashAppOpenFromCacheInternal(activity: Activity, onFinished: Runnable) {
        ...
        ad.show(activity)
        AdsLoadLog.shown("splash app open ad", AdUnitIds.Rc.SPLASH_APP_OPEN, AdUnitIds.splashAppOpen())
    }
}
